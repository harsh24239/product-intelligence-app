...
def create_deck():
    template_path = '[EXT] UniHack-Protoype Template  (2).pptx'
    ...
    DARK_BLUE = RGBColor(15, 23, 42)       # #0F172A (Headings / Primary Text)
    NAVY = RGBColor(30, 58, 138)            # #1E3A8A (Primary Accent)
    CYAN = RGBColor(2, 132, 199)            # #0284C7 (Tech / Subheadings)
    EMERALD = RGBColor(5, 150, 105)         # #059669 (Success / Metrics)
    AMBER = RGBColor(217, 119, 6)           # #D97706 (Highlights / Badges)
    GRAY_TEXT = RGBColor(71, 85, 105)       # #475569 (Body Text)
    MUTED_BG = RGBColor(241, 245, 249)      # #F1F5F9 (Card Background)
    BORDER_COLOR = RGBColor(203, 213, 225)  # #CBD5E1 (Card Border)
    WHITE = RGBColor(255, 255, 255)
    LIGHT_BLUE_BG = RGBColor(238, 242, 255) # #EEF2FF
    LIGHT_GREEN_BG = RGBColor(236, 253, 245)# #ECFDF5

    def clear_text_shapes(slide):
        # Keep shape 0 (background graphic), remove other text boxes / placeholders
        shapes_to_remove = []
    ...
    def add_slide_header(slide, title_text, category_badge=None):
        # Header Box
        header_box = slide.shapes.add_textbox(Inches(0.6), Inches(0.5), Inches(8.8), Inches(0.8))
    ...
    def add_card(slide, left, top, width, height, bg_color=MUTED_BG, border_color=BORDER_COLOR):
    ...
    # Title overlay card
    add_card(s1, Inches(0.6), Inches(1.1), Inches(8.8), Inches(3.9), WHITE, BORDER_COLOR)
    ...
    p3 = tf.add_paragraph()
    p3.text = "Transforming minimal 6-column distributor inputs into verified, schema-compliant 252-column enterprise product intelligence using Gemini 1.5, Vector RAG & ISO/IEC Knowledge Graphs."
    ...
    # Highlights row
    p4 = tf.add_paragraph()
    p4.text = "Key Achievements:  ✓ 100% Schema Accuracy (252 Cols)   ✓ Automated Placeholder Cleansing   ✓ Verified ISO/IEC Compliance   ✓ 0-Error Production Build"
    p4.font.name = "Google Sans"
    p4.font.size = Pt(10)
    p4.font.bold = True
    ...
    add_card(s2, Inches(0.6), Inches(1.5), Inches(8.8), Inches(3.5), WHITE, BORDER_COLOR)
    tbox = s2.shapes.add_textbox(Inches(0.9), Inches(1.7), Inches(8.2), Inches(3.0))
    tf = tbox.text_frame
    tf.word_wrap = True
    ...
    cards_data = [
        ("The Challenge", NAVY, LIGHT_BLUE_BG, [
        ...
        ("Key Impact & Output", EMERALD, LIGHT_GREEN_BG, [
    ...
    for i, (title, color, bg, bullets) in enumerate(cards_data):
        ...
        add_card(s3, left_pos, top_pos, col_w, height, bg, BORDER_COLOR)
    ...
    # ==========================================
    # SLIDE 4: Core Technical Questions Addressed
    # ==========================================
    s4 = prs.slides[3]
    ...
    q_data = [
        ("1. How does it enrich minimal product info?", NAVY, [
        ...
        ("3. What makes it scalable for enterprise catalogs?", CYAN, [
    ...
    for i, (q_title, q_color, q_points) in enumerate(q_data):
        top_y = Inches(1.4) + i * (row_h + Inches(0.12))
        add_card(s4, Inches(0.6), top_y, row_w, row_h, WHITE, BORDER_COLOR)
    ...
    add_card(s5, Inches(0.6), Inches(1.4), w_card, h_card, WHITE, BORDER_COLOR)
    tbox = s5.shapes.add_textbox(Inches(0.8), Inches(1.55), w_card - Inches(0.4), h_card - Inches(0.3))
    tf = tbox.text_frame
    tf.word_wrap = True
    ...
    # Right Card: USPs
    add_card(s5, Inches(5.15), Inches(1.4), w_card, h_card, LIGHT_BLUE_BG, BORDER_COLOR)
    tbox = s5.shapes.add_textbox(Inches(5.35), Inches(1.55), w_card - Inches(0.4), h_card - Inches(0.3))
    tf = tbox.text_frame
    tf.word_wrap = True
    ...
    # ==========================================
    # SLIDE 6: Features Offered by Solution
    # ==========================================
    s6 = prs.slides[5]
    ...
    for idx, (ftitle, fdesc) in enumerate(features):
        ...
        y = Inches(1.4) + row * (f_h + f_gap_y)
        
        add_card(s6, x, y, f_w, f_h, WHITE, BORDER_COLOR)
    ...
    # ==========================================
    # SLIDE 7: Process Flow & Agent Pipeline Diagram
    # ==========================================
    s7 = prs.slides[6]
    ...
    for idx, (stg_num, stg_title, stg_desc) in enumerate(steps_data):
        ...
        bg = LIGHT_GREEN_BG if idx == 4 else (LIGHT_BLUE_BG if idx % 2 == 0 else WHITE)
        add_card(s7, x, y, step_w, step_h, bg, BORDER_COLOR)
    ...
    # ==========================================
    # SLIDE 8: Wireframes & UX Design System
    # ==========================================
    s8 = prs.slides[7]
    ...
    for i, (utitle, ucolor, upoints) in enumerate(ux_cards):
        ...
        y = Inches(1.4)
        add_card(s8, x, y, ux_w, ux_h, WHITE, BORDER_COLOR)
    ...
    arch_tiers = [
        ("Tier 1: Presentation & Visualization (Frontend SPA)", NAVY, LIGHT_BLUE_BG, 
         "React 18 + Vite + TypeScript  |  Tailwind & Custom Glassmorphism CSS  |  Lucide Icons  |  jsPDF Datasheet Engine"),
        ("Tier 2: Orchestration & AI Engine (Node.js / Express)", CYAN, WHITE, 
        ...
        ("Tier 3: Domain Knowledge & Persistence (Data & Template Layer)", EMERALD, LIGHT_GREEN_BG, 
    ...
    for i, (tier_title, tier_col, tier_bg, tier_desc) in enumerate(arch_tiers):
        y = Inches(1.4) + i * (t_h + Inches(0.18))
        add_card(s9, Inches(0.6), y, t_w, t_h, tier_bg, BORDER_COLOR)
    ...
    tech_categories = [
        ("AI & LLM Services", CYAN, ["Google Gemini 1.5 Flash / Pro", "Gemini Vision OCR (VLM)", "Vector Cosine Similarity RAG", "Prompt Engineering Pipeline"]),
    ...
    for idx, (cat_title, cat_col, cat_items) in enumerate(tech_categories):
        ...
        y = Inches(1.4) + r * (tech_h + Inches(0.18))
        
        add_card(s10, x, y, tech_w, tech_h, WHITE, BORDER_COLOR)
    ...
    add_card(s11, Inches(0.6), Inches(1.4), cost_w, cost_h, WHITE, BORDER_COLOR)
    tbox = s11.shapes.add_textbox(Inches(0.8), Inches(1.55), cost_w - Inches(0.4), cost_h - Inches(0.3))
    tf = tbox.text_frame
    tf.word_wrap = True
    ...
    add_card(s11, Inches(5.15), Inches(1.4), cost_w, cost_h, LIGHT_GREEN_BG, BORDER_COLOR)
    tbox = s11.shapes.add_textbox(Inches(5.35), Inches(1.55), cost_w - Inches(0.4), cost_h - Inches(0.3))
    tf = tbox.text_frame
    tf.word_wrap = True
    ...
    for idx, (caption, img_path) in enumerate(shots):
        ...
        y = Inches(1.35) + r * (img_h + Inches(0.22))
        
        add_card(s12, x, y, img_w, img_h, WHITE, BORDER_COLOR)
        if os.path.exists(img_path):
            try:
                s12.shapes.add_picture(img_path, x + Inches(0.05), y + Inches(0.05), img_w - Inches(0.1), img_h - Inches(0.28))
    ...
    roadmap = [
        ...
        ("Phase 2: Automated Web Crawling (Q3 2026)", CYAN, [
    ...
    for idx, (rtitle, rcol, rbullets) in enumerate(roadmap):
        ...
        add_card(s13, x, y, rm_w, rm_h, WHITE, BORDER_COLOR)
    ...
    add_card(s14, Inches(0.6), Inches(1.4), Inches(8.8), Inches(3.6), WHITE, BORDER_COLOR)
    tbox = s14.shapes.add_textbox(Inches(0.9), Inches(1.6), Inches(8.2), Inches(3.2))
    tf = tbox.text_frame
    tf.word_wrap = True
    ...
    add_card(s15, Inches(0.6), Inches(1.1), Inches(8.8), Inches(3.9), WHITE, BORDER_COLOR)
